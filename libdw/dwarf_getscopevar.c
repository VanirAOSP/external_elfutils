...
getattr (Dwarf_Die *die, int search_name, Dwarf_Word *value)
{
  ...
  static inline bool file_matches (const char *match_file, const char *lastfile, bool *lastfile_matches, size_t match_file_len, Dwarf_Files *files, size_t idx)
    {
      if (idx >= files->nfiles)
	return false;
		...
		if (match_file != NULL)
		  {
		    ...
		    if (!file_matches (match_file, lastfile, &lastfile_matches, match_file_len, files, i))
		      break;
